...
def load_service_account_json():
    ...
    try:
        ...
        def validate_json(parsed_json):
            ...
            if not parsed_json['private_key'].strip().startswith('-----BEGIN PRIVATE KEY-----'):
        ...
        if os.path.exists(creds_file):
            ...
            try:
                ...
                raise ValueError(f"Invalid JSON in credentials file: {str(je)}")
        else:
            # Fallback to environment variable
            logger.info("Credentials file not found, checking environment variable")
            service_account_json = os.getenv('GOOGLE_SERVICE_ACCOUNT_JSON')
            if not service_account_json:
                logger.error("No credentials found in file or environment variable")
                raise ValueError("No credentials found in file or environment variable")
                
            try:
                # Clean the JSON string before parsing
                service_account_json = service_account_json.strip()
                # Remove any unwanted Unicode characters
                service_account_json = ''.join(char for char in service_account_json 
                                            if char.isprintable() or char in ['\n', '\r', '\t'])
                parsed_json = json.loads(service_account_json)
            except json.JSONDecodeError as je:
                logger.error(f"Failed to parse JSON from environment: {str(je)}")
                logger.error(f"JSON content preview: {service_account_json[:50]}...")
                raise ValueError(f"Invalid JSON in environment variable: {str(je)}")

        # Validate the JSON structure
        parsed_json = validate_json(parsed_json)

        # Convert back to JSON string with proper escaping
        service_account_json = json.dumps(parsed_json, ensure_ascii=False, indent=None, separators=(',', ':'))
...
def check_user_access(sheet_id: str, username: str) -> bool:
    ...
    try:
        ...
        return username.lower() in users_df['Name'].astype(str).str.lower().tolist()
    ...
    if st.session_state.selected_sheet:
        ...
        try:
            ...
            if has_inputs:
                ...
                # Process INPUTS form
                try:
                    st.session_state.form_service.handle_inputs_sheet(selected_sheet['id'])
                except Exception as e:
            ...
            if has_users_sheet and st.session_state.get('is_logged_in', False):
                try:
                    ...
                    if not users_df.empty:
                        ...
                        user_row = users_df[users_df['Name'].str.lower() == username]
